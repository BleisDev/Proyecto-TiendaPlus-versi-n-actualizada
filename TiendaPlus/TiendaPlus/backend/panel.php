...
if (!isset($conn) || $conn->connect_error) {
    die("Error crítico: No hay conexión a la base de datos. " . ($conn->connect_error ?? ''));
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($accion === 'crear' || $accion === 'editar')) {
    ...
    try {
        if ($tabla === 'usuarios') {
            ...
            if ($accion === 'crear') {
                $stmt = $conn->prepare("INSERT INTO carrito (usuario_id, producto_id, cantidad, talla, fecha_agregado) VALUES (?, ?, ?, ?, NOW())");
                $stmt->bind_param("iiis", $usuario_id, $producto_id, $cantidad, $talla);
            } else {
                $stmt = $conn->prepare("UPDATE carrito SET usuario_id=?, producto_id=?, cantidad=?, talla=? WHERE id=?");
                $stmt->bind_param("iiisi", $usuario_id, $producto_id, $cantidad, $talla, $id);
            }
            ...
            if ($accion === 'crear') {
                $stmt = $conn->prepare("INSERT INTO pedidos (usuario_id, fecha, total, envio, estado) VALUES (?, NOW(), ?, ?, ?)");
                $stmt->bind_param("idds", $usuario_id, $total, $envio, $estado);
            } else {
                $stmt = $conn->prepare("UPDATE pedidos SET usuario_id=?, total=?, envio=?, estado=? WHERE id=?");
                $stmt->bind_param("iddsi", $usuario_id, $total, $envio, $estado, $id);
            }
            ...
            if ($accion === 'crear') {
                $stmt = $conn->prepare("INSERT INTO detalle_pedido (pedido_id, producto_id, cantidad, precio, talla) VALUES (?, ?, ?, ?, ?)");
                $stmt->bind_param("iiids", $pedido_id, $producto_id, $cantidad, $precio, $talla);
            } else {
                $stmt = $conn->prepare("UPDATE detalle_pedido SET pedido_id=?, producto_id=?, cantidad=?, precio=?, talla=? WHERE id=?");
                $stmt->bind_param("iiidsi", $pedido_id, $producto_id, $cantidad, $precio, $talla, $id);
            }
...
// Consultas para listar
if ($accion === 'listar') {
    function ejecutar_consulta($conn, $sql, $nombre_tabla)
    {
        $resultado = $conn->query($sql);
        if (!$resultado) {
            echo "<div class='alert alert-error'>Error al cargar $nombre_tabla: " . $conn->error . "</div>";
            return false;
        }
        return $resultado;
    }

    $usuarios = ejecutar_consulta($conn, "SELECT * FROM usuarios", "usuarios");
    $categorias = ejecutar_consulta($conn, "SELECT * FROM categorias", "categorias");
    $productos = ejecutar_consulta($conn, "SELECT p.*, c.nombre AS categoria FROM productos p LEFT JOIN categorias c ON p.categoria_id = c.id", "productos");
    $carritos = ejecutar_consulta($conn, "SELECT ca.*, u.nombre AS usuario, p.nombre AS producto FROM carrito ca LEFT JOIN usuarios u ON ca.usuario_id = u.id LEFT JOIN productos p ON ca.producto_id = p.id", "carrito");
    $pedidos = ejecutar_consulta($conn, "SELECT pe.*, u.nombre AS usuario FROM pedidos pe LEFT JOIN usuarios u ON pe.usuario_id = u.id", "pedidos");
    $detalle_pedido = ejecutar_consulta($conn, "SELECT dp.*, p.nombre AS producto FROM detalle_pedido dp LEFT JOIN productos p ON dp.producto_id = p.id", "detalle_pedido");
    $resenas = ejecutar_consulta($conn, "SELECT r.*, u.nombre AS usuario, p.nombre AS producto FROM resenas r LEFT JOIN usuarios u ON r.usuario_id = u.id LEFT JOIN productos p ON r.producto_id = p.id", "resenas");
}
?>
...
<head>
    ...
    <style>
        * {
        ...
        .btn-logout:hover {
            background: #d4a574 !important;
            border-color: #d47474ff !important;
            color: #693333ff !important;
        }
            ...
            <?php if ($tabla === 'dashboard'): ?>
                ...
                <div class="dashboard-grid">
                    <div class="stat-card">
                        ...
                        <h3>Usuarios</h3>
                        <div class="number">
                            <?= $conn->query("SELECT COUNT(*) as total FROM usuarios")->fetch_assoc()['total'] ?>
                        </div>
                    </div>
                    <div class="stat-card">
                        ...
                        <h3>Productos</h3>
                        <div class="number">
                            <?= $conn->query("SELECT COUNT(*) as total FROM productos")->fetch_assoc()['total'] ?>
                        </div>
                    </div>
                    <div class="stat-card">
                        ...
                        <h3>Pedidos</h3>
                        <div class="number">
                            <?= $conn->query("SELECT COUNT(*) as total FROM pedidos")->fetch_assoc()['total'] ?>
                        </div>
                    </div>
                    <div class="stat-card">
                        ...
                        <h3>Categorías</h3>
                        <div class="number">
                            <?= $conn->query("SELECT COUNT(*) as total FROM categorias")->fetch_assoc()['total'] ?>
                        </div>
                    </div>
                </div>
                    ...
                    <form method="POST">
                        <?php if ($tabla === 'usuarios'): ?>
                            ...
                            <div class="form-group">
                                ...
                                <input type="password" name="password" class="form-control"
                                    placeholder="<?= $accion === 'editar' ? 'Dejar en blanco para mantener actual' : '' ?>"
                        ...
                        <?php elseif ($tabla === 'productos'): ?>
                            ...
                            <div class="form-group">
                                ...
                                <small style="color: #666; font-size: 12px;">Separar tallas con comas. Dejar vacío si no
                                    aplica.</small>
                            </div>
                            <div class="form-group">
                        ...
                        <?php elseif ($tabla === 'carrito'): ?>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="talla" class="form-control" value="<?= $registro['talla'] ?? '' ?>"
                                    placeholder="Ej: M, L, XL (opcional)">
                                <small style="color: #666; font-size: 12px;">Dejar vacío si no aplica.</small>
                            </div>
                        ...
                        <?php elseif ($tabla === 'pedidos'): ?>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="envio" class="form-control" value="<?= $registro['envio'] ?? '0' ?>"
                                    step="0.01">
                                <small style="color: #666; font-size: 12px;">Costo de envío (0 si no aplica).</small>
                            </div>
                            <div class="form-group">
                    ...
                    <div class="table-responsive">
                        ...
                        <?php elseif ($tabla === 'carrito'): ?>
                            <table>
                                ...
                                <tbody>
                                    <?php if ($carritos && $carritos->num_rows > 0): ?>
                                        <?php while ($ca = $carritos->fetch_assoc()): ?>
                                            <tr>
                                                <td><?= $ca['id'] ?></td>
                                                <td><?= $ca['usuario_id'] ?></td>
                                                <td><?= $ca['producto_id'] ?></td>
                                                <td><?= $ca['cantidad'] ?></td>
                                                <td><?= $ca['talla'] ?></td>
                                    ...
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="7" style="text-align:center;">El carrito está vacío</td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                                    ...
                                    <?php if ($pedidos && $pedidos->num_rows > 0): ?>
                                        <?php while ($pe = $pedidos->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td><?= $pe['estado'] ?></td>
                                                <td>
                                                    <button onclick="toggleDetallesPedido(<?= $pe['id'] ?>)" class="btn btn-info btn-sm"
                                                        title="Ver Detalles">
                        ...
                        <?php elseif ($tabla === 'detalle_pedido'): ?>
                            <table>
                                ...
                                <tbody>
                                    <?php if ($detalle_pedido && $detalle_pedido->num_rows > 0): ?>
                                        <?php while ($dp = $detalle_pedido->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td><?= $dp['talla'] ? $dp['talla'] : '<span style="color:#999;">N/A</span>' ?></td>
                                                <td>
                                    ...
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="7" style="text-align:center;">No hay detalles de pedidos registrados</td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
